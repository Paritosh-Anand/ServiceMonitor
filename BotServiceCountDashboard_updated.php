...
$listquery = "SELECT datacenter, service_name, service_threshold FROM service_config WHERE service_type = 'bot'";
$botrs = queryMysql($query, $debug, $link);
...
<td class='dc'>
	...
	$conf = mysqli_fetch_object($botrs);
	while($row && $conf) {
		if($row->datacenter == "SVS") {
			$bg_color = ($row->service_count < $_BOT_SERVICE_INFO[$conf->service_threshold])?'red':(($row->service_count > $_BOT_SERVICE_INFO[$conf->service_threshold])?'#EAB988':'');
			print "<tr><td>" . $row->datacenter . "</td><td class ='tooltip' onclick='getZKData(\"" . $row->service_name . "\",\"" . $row->datacenter . "\",\"" . $fabric . "\")'>" . $row->service_name . "</td><td style='background-color:" . $bg_color . "'>" . $row->service_count . " [" . $_BOT_SERVICE_INFO[$conf->service_threshold] . "]</td></tr>";
		}
	}
	?>
	</table>
