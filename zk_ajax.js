...
var hide = function () {
	...
	document.getElementById("add_new_service").style.display ="none";
	document.getElementById("threshold_popup").style.display ="none";
	document.getElementById("mask").style.display ="none";
	document.getElementById("response_data").innerHTML = "Loading Data";
}

var setFormVal = function(id, service_name, service_threshold){
	document.getElementById("service_name").value 		= service_name;
	document.getElementById("service_threshold").value 	= service_threshold;
	document.getElementById("service_id").value 		= id;
	document.getElementById("threshold_popup").style.display="block";
	document.getElementById("mask").style.display="block";
}


var validateEditForm = function(service_name,service_threshold){
	var retVal = true;
	if(retVal && service_name.value == ""){
		alert("Service name can't be empty");
		retVal = false;
	}
	if(retVal && (isNaN(service_threshold.value) || service_threshold > 0 || service_threshold.value == "")){
		alert("Service threshold can't be empty or 0");
		retVal = false;
	}
	return retVal;
}

var addFormVal = function(service_type){
	document.getElementById("add_new_service").style.display="block";
	document.getElementById("mask").style.display="block";
